...
Function MouseReport {
    $script:MouseButton = $null; $script:MouseButton = (Get-ItemProperty "HKCU:\Control Panel\Mouse").SwapMouseButtons
...
MouseReport
$Choice = @('Y', 'X'); Do {
    Write-Host "  Swap?: (Y) / (X): " -NoNewline -ForegroundColor Yellow
    $Swap = (Read-Host).Trim()
} Until (($Choice.Contains($Swap.ToUpper()))); If ($Swap -eq "x") { break }
